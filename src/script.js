      ...
      if (keyValue === "𝑥²") {
        ...
        if (display.textContent !== "0") {
          if (display.textContent !== "invalid input") {
            ...
            smallDisplaySuccessiveExpression(
              "-",
              displayValue,
              previousKeyType
            );
...
function smallDisplaySuccessiveExpression(
  expression,
  displayValue,
  previousKeyType
) {
...
function formatDecimal(value) {
  ...
  var decimal = valueNum - Math.floor(valueNum),
    length = decimal.toString().length;
  ...
  if (diff <= 31) {
    if (arrowDirection === "left") {
      if (diff + translationValue < 30) {
        smallDisplay.parentNode.style.transform +=
          "translateX(" + translationValue.toString() + "px" + ")";
      } else {
        const newDiff = 30 - diff;
        smallDisplay.parentNode.style.transform +=
          "translateX(" + newDiff.toString() + "px" + ")";
      }
    } else {
      if (
        diff + smallDisplay.parentNode.offsetWidth - translationValue >
        calculator.offsetWidth - 30
      ) {
        smallDisplay.parentNode.style.transform +=
          "translateX(" + "-" + translationValue.toString() + "px" + ")";
      } else {
        var newDiff =
          diff +
          smallDisplay.parentNode.offsetWidth -
          (calculator.offsetWidth - 30);
        smallDisplay.parentNode.style.transform +=
          "translateX(" + "-" + newDiff.toString() + "px" + ")";
      }
...
function updateDisplayFontSize() {
  ...
  console.log("cal width : " + calculator.offsetWidth);
